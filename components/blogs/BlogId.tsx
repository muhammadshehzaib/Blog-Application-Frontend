...
const BlogId = ({ blog }) => {
    ...
    return (
        <div className="max-w-2xl mx-auto mt-8">
            {blogs && (
                <>
                    <img src={blogs.image} alt={blogs.title} className="rounded-md w-full h-64 object-cover mb-4" />
            ...
            <div className="mt-8">
                ...
                <div className="flex mb-4">
                    ...
                    />
                    <button onClick={handleComment} className="bg-blue-500 text-white px-4 py-2 rounded-md">Submit</button>
                </div>
                {blogs?.comments.length !== 0 ? (
                    <div>
                        {blogs?.comments.map((comment) => (
                            <Comments key={comment.id} comment={comment.comment} blogId={blog} />
                        ))}
                    </div>
                ) : (
