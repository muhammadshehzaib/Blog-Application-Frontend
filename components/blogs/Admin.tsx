...
const Admin = ({ blog }) => {
    ...
    const handleApprove = async (_id: any) => {
        ...
        const response = await fetch(`http://localhost:3002/blogs/approved/${blog._id}`, {
            method: "PUT",
            headers: {
    ...
    const handleDisapprove = async (_id: any) => {
        const response = await fetch(`http://localhost:3002/blogs/disapproved/${blog._id}`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json",
                "authorization": `bearer ${token}`
    ...
    return (
        <div>
            <tr key={blog.id} className="even:bg-gray-50">
                ...
                <td className="py-2 px-4 border-b">{blog.createdAt}</td>
